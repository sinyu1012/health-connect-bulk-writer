...
object HealthConnectCatalog {
    val recordTypes: Set<KClass<out Record>> = linkedSetOf(
    ...
    val featureBasedExtraPermissions: Map<String, Int> = mapOf(
    ...
    val exerciseRoutePermissions: Set<String> = setOf(
        HealthPermission.PERMISSION_READ_EXERCISE_ROUTES,
        HealthPermission.PERMISSION_WRITE_EXERCISE_ROUTE,
    )

    fun permissionsFor(recordTypes: Iterable<KClass<out Record>>): Set<String> {
        val readWrite = recordTypes.flatMap { recordType ->
            listOf(
                HealthPermission.getReadPermission(recordType),
                HealthPermission.getWritePermission(recordType),
            )
        }
        return (readWrite + exerciseRoutePermissions).toSet()
    }

    fun writePermissionsFor(recordTypes: Iterable<KClass<out Record>>): Set<String> =
        recordTypes.mapTo(linkedSetOf()) { recordType ->
            HealthPermission.getWritePermission(recordType)
        }
}
